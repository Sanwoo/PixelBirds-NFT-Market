...
import { TOKEN_ADDRESS, NFT_MARKET_ADDRESS } from "../assets/config";
  ...
  const handleBuy = async () => {
    try {
      if (allowance < parseEther(nft.price)) {
        ...
        showModal(
          "Token Approval",
          "Approving, it may take few minutes.",
          approveTx
        );
      ...
      try {
        ...
        showModal(
          "Purchase NFT",
          "Purchasing, it may take few minutes.",
          buyTx
        );
        await waitForTransactionReceipt(config, { hash: buyTx }).then((res) => {
